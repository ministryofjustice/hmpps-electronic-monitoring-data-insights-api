...
WITH days AS (
    SELECT
        d AS day_date,
        date_diff('day', DATE '2026-02-01', d) + 1 AS day_no
    FROM UNNEST(
        SEQUENCE(DATE '2026-02-01', DATE '2026-02-28', INTERVAL '1' DAY)
    ) AS x(d)
),
route_for_day AS (
    SELECT
        day_date,
        day_no,
        1 + CAST(((day_no + 1 - 1) % 7) AS integer) AS route_id
    FROM days
),
route_points AS (
    SELECT *
...
SELECT
    ...
    ping_ts + INTERVAL '6' MINUTE,
    format(
        'position_london_walk_%s_v6_feb.jsonl',
    ...
    format(
        'allied/mdss/position/%s/position_london_walk_%s_v6_feb.jsonl',
        CAST(date(ping_ts) AS varchar),
        replace(CAST(date(ping_ts) AS varchar), '-', '')
    ),
    format('synthetic-london-walk-v6-feb-%s', CAST(date(ping_ts) AS varchar)),
